...
def _pgd_whitebox(model,
    ...
    gaussian = normal.Normal(0.0, 0.1)
    # pdb.set_trace()
    X_pgd = Variable(X.data, requires_grad=True)
    # stepsize = 2.5 * epsilon / num_steps
    for _ in range(num_steps):
        ...
        # pdb.set_trace()
        # grad_norm = torch.clamp(torch.sqrt(torch.sum(torch.pow(total_grad,2), dim = (2,3), keepdim=True)), 1e-8, numpy.inf)
        
        eta = step_size * total_grad.sign()
