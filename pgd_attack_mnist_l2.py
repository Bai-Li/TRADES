...
def _pgd_whitebox(model,
    ...
    gaussian = normal.Normal(0.0, 0.15)
    # pdb.set_trace()
    ...
    for _ in range(num_steps):
        ...
        for __ in range(num_iter):
            X_pgd_temp= Variable(X_pgd.data + gaussian.sample(X.size()).to(device), requires_grad=True)
            opt = optim.SGD([X_pgd_temp], lr=1e-3)
            opt.zero_grad()

            with torch.enable_grad():
                loss = nn.CrossEntropyLoss()(model(X_pgd_temp), y)
            loss.backward()
            total_grad += X_pgd_temp.grad.data
        # eta = step_size * X_pgd.grad.data.sign()
        total_grad = total_grad / num_iter
        # pdb.set_trace()
        grad_norm = torch.clamp(torch.sqrt(torch.sum(torch.pow(total_grad,2), dim = (2,3), keepdim=True)), 1e-8, np.inf)
        
        eta = step_size * total_grad / grad_norm
        X_pgd = Variable(X_pgd.data + eta, requires_grad=True)
        eta = Variable(X_pgd.data - X.data, requires_grad=True)
        # pdb.set_trace()
        eta_norm = torch.sqrt(torch.sum(torch.pow(eta.data,2), dim = (2,3), keepdim=True))
        eta_final_norm = torch.clamp(eta_norm, 0, epsilon)
        X_pgd = Variable(X.data + eta_final_norm * eta / eta_norm, requires_grad=True)
    X_pgd = Variable(torch.clamp(X_pgd, 0, 1.0), requires_grad=True)
    err_pgd = (model(X_pgd).data.max(1)[1] != y.data).float().sum()
    
    print(torch.max(torch.sqrt(torch.sum(torch.pow(X_pgd-X,2), dim = (2,3))).squeeze()))
    print('err pgd (white-box): ', err_pgd)
    
    return err, err_pgd, X_pgd.detach().cpu().numpy()
...
def eval_adv_test_whitebox(model, device, test_loader):
    ...
    for data, target in test_loader:
        ...
        err_natural, err_robust, X_pgd = _pgd_whitebox(model, X, y)
        robust_err_total += err_robust
        natural_err_total += err_natural
    X_adv = np.concatenate(X_pgd, axis=0)
    np.save("data_attack/X_adv.npy", X_adv)
